    ...
    try{
        ...
        if(decoded && decoded.username == req.params.username || decoded && decoded.admin){
            if(req.query.type == 'liked'){
                ...
                user = await User.findOne({username:req.params.username},{password:0}).populate({path: 'posts', populate:{path:'comments', select:'_id'}}).populate({path: 'liked', select: 'title createdAt comments', populate:{path: 'comments', select: '_id'}})
            }
            //if user = null then "user not found"
            res.send({success: true, user})
        }else{
            user = await User.findOne({username:req.params.username},{email:0,password:0}).populate({path: 'posts', populate:{path:'comments', select:'_id'}}).populate({path: 'liked', select: 'title createdAt comments', populate:{path: 'comments', select: '_id'}})
            res.send({success: true, user})
        }
    }catch(err){
        console.log(err)
